<div style="background-color: rgb(59, 24, 24); height: 100px;"></div>

<!-- breadcam_area_start -->
<div class="opacity_img" style="background-image: url('<?php echo __DOM__; ?>img/banner/2.jpg'); width: 100%; height: 100vh; background-repeat: no-repeat;background-size: cover;">
    <div class="container">
        <div class="row" style="padding-top: 150px;">
            <div class="col-xl-12 text-center">
                <div class="section_title mb-60">
                    <div class="breadcam_text">
                    <h3 style="font-family: 'Lobster', cursive; color: white;">¡Echo!</h3>
                </div>
                <p style="font-weight: bold; color: aquamarine;">Se a realizado la orden :),<a href="<?php echo __DOM__; ?>page/menu" style="color: red;"> Ir al menú.</a></p>
            </div>
        </div>
        </div>
    </div>
</div>